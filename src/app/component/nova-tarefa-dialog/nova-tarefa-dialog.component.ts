...
export class NovaTarefaDialogComponent {
  ...
  salvar(tarefaForm: any) {
    if (tarefaForm.invalid) {
      this.snackBar.open('Por favor, preencha todos os campos obrigatórios.', 'Fechar', { duration: 10000 });
      return;
    }

    if (new Date(this.novaTarefa.dataPrevistaConclusao) < new Date()) {
      this.snackBar.open('A data prevista de conclusão não pode ser anterior à data atual.', 'Fechar', { duration: 10000 });
      return;
    }
